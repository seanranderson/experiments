...
[hndl.PN,hndl.SRate] = audioread('PinkNoise.wav');

if isempty(invfilter)
    % do nothing
else
    % filter with inverse filter
    hndl.PN = ApplyInvFilter(hndl.PN,hndl.SRate,invfilter);
end
...
uicontrol('Style','text','String','Amplitude:', ...
    'FontSize',32,...
    'FontWeight', 'bold', ...
    'Units', 'normalized', 'Position',[0.5,0.1,0.2,0.15]);
hndl.Fig.OutLevel = uicontrol('Style','edit', ...
    ...
    'callback',@SetParams,...
    'Units', 'normalized', 'Position',[0.75,0.1,0.2,0.2]);
...
hndl.Fig.LevelChange = uicontrol('Style', 'edit', ...
    ...
    'backgroundcolor',hndl.colors.white, ...
    'callback',@SetParams,...
    'Units', 'normalized', 'Position',[0.25,0.1,0.2,0.2]);
...
function SetParams(varargin)
    ...
    hndl.PN = (hndl.PN ./ rms(hndl.PN)) * hndl.OutLevel;
